...
function QuestionCard({ q, value, setValue, feedback, onCheck }) {
  return (
    <Card sx={{ mb: 3, borderRadius: '20px', boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
      <CardContent sx={{ p: 3 }}>
        <Typography sx={{ fontWeight: 600, mb: 1.5 }}>Q{q.qid}. {q.text}</Typography>
        {q.options.map((opt, idx) => (
          <label key={idx} style={{ display: 'flex', gap: 8, padding: '6px 0' }}>
            <input type="radio" name={`q-${q.qid}`} checked={value === idx} onChange={() => setValue(idx)} />
            <span>{opt}</span>
          </label>
        ))}
        <Box sx={{ mt: 1.5, display: 'flex', gap: 2, alignItems: 'center' }}>
          <Button size="small" variant="contained" onClick={() => onCheck(q.qid, value)}
                  sx={{ borderRadius: '12px', background: 'linear-gradient(135deg,#454a59 0%,#6e7687 100%)' }}>
            Check
          </Button>
          {feedback && (
...
export default function QuizRun() {
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: 'auto', p: 3 }}>
      <Typography variant="h5" sx={{ fontWeight: 700, mb: 2 }}>Attempt #{attempt}</Typography>
      {qs.map(q => (
        <QuestionCard key={q.qid} q={q}
          value={answers[q.qid]}
